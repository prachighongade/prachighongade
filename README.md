...
**AgroMind** -- Smart farming assistant combining ensemble ML with a multilingual chatbot, so farmers get crop and weather guidance in their own language. ``Random Forest`` ``XGBoost`` ``CNN`` ``LSTM`` ``Node.js`` ``Firebase``
...
\[\*\*AskSQL\*\*](https://github.com/prachighongade/Ask-SQL-) -- Natural language to SQL engine. Ask a question in plain English, get a query, an explanation, and the result. `FastAPI` `DuckDB` `Llama 3.3 70B` `React`AgroMind — Smart farming assistant combining ensemble ML with a multilingual chatbot, so farmers get crop and weather guidance in their own language. Random Forest XGBoost CNN LSTM Node.js Firebase
